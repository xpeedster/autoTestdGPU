...
int main(int argc, char** argv)
{
	...
	printf("hints-CPU with size %d and AccessedBy\n", elm);
	...
	CHECK(cudaMemAdvise(A, N, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
	printf("cudaCpuDeviceId = %d\n", cudaCpuDeviceId);
	
	if(accBy == 1){
		printf("hints-CPU with size %d and AccessedBy\n", N);
		CHECK(cudaMemAdvise(A, N, cudaMemAdviseSetAccessedBy, 0));
	}else{
		printf("hints-CPU with size %d and NOT AccessedBy\n", N);
	}
	
	// Initialize data
	std::system("ps -C hints-CPU -o maj_flt");
	for(int i = 0; i < elm; i++){
		A[i] = 1;
	}
	std::system("ps -C hints-CPU -o maj_flt");
	
	// Allocated size is 64 KB, hence, 10 iterations should result in 10 maj_flts if data does NOT stay in CPU.
	...
	pf_after = exec("ps -C hints-CPU -o maj_flt");
